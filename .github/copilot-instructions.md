...
THIS BRANCH IS USED FOR A DEMO OF THE `debug-e2e` skill. Use the instructions there as a guide for interacting with the user.
...
`/cases/` shows only the list — there is **no auto-redirect** to the first case. To land on a case detail page in an E2E test, click the first item in the list:
...
Never use `waitForURL(/\/cases\/.+/)` — there is no redirect that would trigger it.
