...
- TypeScript 5.x, React 18.3.x + KendoReact components (Input, Select, Button), Kendo icons, Tailwind CSS, Zod (validation) (004-inline-edit-components)
- N/A (components are presentational; save handled by consumer callbacks) (004-inline-edit-components)

- TypeScript 5.x (via Node.js runtime in devcontainer) + React 18, Vite, tRPC, React Query, KendoReact, Tailwind CSS (002-header-menu-components)
...
- **KendoReact Components**:
  - Always prioritize using KendoReact components over native HTML elements (e.g., use KendoReact Select instead of `<select>`, KendoReact Input instead of `<input>`, etc.)
  - If a needed component is not available, install the KendoReact equivalent
  - KendoReact components should be installed to `packages/client/src/components/ui/` directory and exported via `packages/client/src/components/ui/index.ts`
- **Custom Components**: If a custom component must be built on top of underlying KendoReact components (e.g., EditableSelect, ConfirmationDialog), it should go in `packages/client/src/components/common/`
...
- 004-inline-edit-components: Added TypeScript 5.x, React 18.3.x + KendoReact components (Input, Select, Button), Kendo icons, Tailwind CSS, Zod (validation)

- 002-header-menu-components: Added TypeScript 5.x (via Node.js runtime in devcontainer) + React 18, Vite, tRPC, React Query, KendoReact, Tailwind CSS
