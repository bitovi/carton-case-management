...
| `figma-from-code`                | **Orchestrator** — thin dispatcher that delegates all phases to subagents, reads only small summary files, tracks state. Never calls use_figma directly          | Starting or resuming a full Figma rebuild from the codebase                                                     |
| `figma-setup-variables`          | Extracts CSS/Tailwind tokens and creates Figma variable collections. Called by figma-from-code-setup-tokens                                                      | When figma-from-code-setup-tokens runs, or standalone when tokens change                                        |
| `figma-setup-file-structure`     | Creates Figma page skeleton and foundations docs. Called by figma-from-code-setup-structure                                                                      | When figma-from-code-setup-structure runs, or standalone to set up a fresh file                                 |
