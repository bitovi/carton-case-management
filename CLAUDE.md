...
| `figma-from-code`                 | **Orchestrator** — runs all phases (discovery → tokens → file structure → pre-capture → components → screens → validate), tracks state, delegates to the right skill per phase                        | Starting or resuming a full Figma rebuild from the codebase                                                     |
| `figma-from-code-discovery-components` | Phase 0a — discovers runtime component hierarchy and tiered build order via map-components.js against the live dev server                                                                       | When the orchestrator runs Phase 0a, or standalone to audit site component architecture                         |
| `figma-from-code-discovery-assets` | Phase 0b — discovers all Lucide icons and SVG assets used in the codebase, extracts SVG data, maps per-component usage                                                                             | When the orchestrator runs Phase 0b, or standalone to audit icon/asset usage                                    |
| `figma-from-code-precapture`      | Phase 2.5 — captures app screenshots and text content for all UI components via parallel haiku subagents running screenshot.js and extract-text.js in batch mode                                     | When the orchestrator runs Phase 2.5, or standalone to refresh app screenshots                                  |
| `figma-from-code-build-tier`      | Phase 3 — builds all Figma components for a tier by dispatching parallel sonnet subagents. Handles icon/asset preamble, library filtering, subagent prompts, result collection                       | When the orchestrator runs Phase 3, or standalone to build/rebuild a tier                                       |
| `figma-from-code-build-screens`   | Phase 4 — builds full-page 1440x900 screen frames by composing built component instances, using pre-captured app screenshots as reference                                                           | When the orchestrator runs Phase 4, or standalone to rebuild screen layouts                                     |
| `figma-from-code-validator`       | Validates a `figma-from-code` run — lists all Figma components, screenshots each Storybook story alongside its Figma counterpart, reports components without stories                                 | After running `figma-from-code` to audit correctness                                                            |
...
| `figma-from-code-build-component` | Build a single Figma component from code with visual validation. Accepts screenshots, variants, source code, styles, icons, and built components. Iterates up to 3 times to match the app screenshot | Called by `figma-from-code` Phase 3 per component, or standalone for building/updating individual components    |
| `screenshot-comparison`           | Pixel-level visual diff of any two screenshots with LLM-generated difference description. Includes evaluation tool with synthetic test cases                                                         | When comparing two screenshots for visual fidelity (design vs implementation, regression testing, before/after) |
