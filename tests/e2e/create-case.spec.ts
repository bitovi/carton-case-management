...
//     const createButton = page.getByRole('button', { name: 'Create Case' }).first();
...
//     await page.goto('/cases/');
//     const caseListLinks = page.locator('.flex.flex-col.gap-2 a');
//     await expect(caseListLinks.first()).toBeVisible({ timeout: 10000 });

//     const initialCaseCount = await caseListLinks.count();
...
//     await expect(caseListLinks.first()).toBeVisible({ timeout: 5000 });
//     const finalCaseCount = await caseListLinks.count();
