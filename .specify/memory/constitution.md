...
- **Component Library Usage (STRICTLY ENFORCED)**: All component changes and additions MUST follow this hierarchy: (1) Use existing components from `packages/client/src/ui/` first, (2) If needed component doesn't exist locally, add it from KendoReact component registry, (3) Only create custom components when KendoReact does not provide that functionality. All new components added to the ui library must adhere to the Figma design system specifications for that component type.
...
- **KendoReact Integration**: Use SKendoReact patterns for consistency
- **Accessibility**: Kendo Components meet accessibility standards
