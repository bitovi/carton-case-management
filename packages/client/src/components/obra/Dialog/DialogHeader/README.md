...
| ---------- | ------------ | -------------------------------------------------- | ---------------- | ---------------------------------------------------- |
| Close Icon | SVG assets   | Kendo React xIcon                                  | DialogHeader.tsx | Project convention, consistent with other components |
...
<DialogHeader type="Header" title="Settings" onClose={() => setIsOpen(false)} />;
```
...
```tsx
<DialogHeader type="Close Only" onClose={() => setIsOpen(false)} />
```

### With Custom Styling

```tsx
<DialogHeader type="Header" title="Edit Profile" onClose={handleClose} className="border-b pb-4" />
```
