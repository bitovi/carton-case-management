...
describe('CaseList', () => {
  ...
  it('filters cases by title as the user types', async () => {
    ...
    await user.type(screen.getByPlaceholderText('Search cases...'), 'second');
  ...
  it('filters cases by case number as the user types', async () => {
    ...
    // First case number: #CAS-240115-1
    await user.type(screen.getByPlaceholderText('Search cases...'), '240115');

    expect(screen.getByText('First Case')).toBeInTheDocument();
    expect(screen.queryByText('Second Case')).not.toBeInTheDocument();
  });

  it('shows a no-matches message when the search has no results', async () => {
    const user = userEvent.setup();
    ...
    await user.type(screen.getByPlaceholderText('Search cases...'), 'nonexistent case');
