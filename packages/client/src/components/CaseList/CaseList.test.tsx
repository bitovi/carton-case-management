...
describe('CaseList', () => {
  ...
  it('renders search input above the case list', async () => {
    server.use(
      http.get('/trpc/case.list*', () => {
        return HttpResponse.json({ result: { data: mockCases } });
      })
  ...
  it('shows empty search message when no cases match the query', async () => {
    const user = userEvent.setup();
    ...
    await user.type(screen.getByPlaceholderText('Search cases...'), 'xyznotfound');
