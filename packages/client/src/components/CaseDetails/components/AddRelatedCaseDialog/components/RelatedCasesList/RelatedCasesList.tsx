import { cn } from '@/lib/utils';
import { Checkbox } from '@/components/obra';
import type { RelatedCasesListProps } from './types';

export function RelatedCasesList({
  title = 'Add Related Cases',
  cases,
  onCaseToggle,
  className,
}: RelatedCasesListProps) {
  return (
    <div
      className={cn(
        'flex w-[342px] flex-col gap-4 px-4 pb-4 pt-0.5',
        className
      )}
    >
      <h2 className="text-xl font-semibold leading-6">{title}</h2>
      {cases.map((caseItem) => (
        <div key={caseItem.id} className="flex items-center">
          <Checkbox
            checked={caseItem.selected}
            onCheckedChange={() => onCaseToggle(caseItem.id)}
          />
          <div className="flex flex-1 flex-col rounded-lg px-4 py-2 text-sm leading-[21px]">
            <p className="font-semibold text-teal-600">{caseItem.title}</p>
            <p className="text-gray-950">{caseItem.caseNumber}</p>
          </div>
        </div>
      ))}
    </div>
  );
}
