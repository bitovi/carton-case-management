...
- **code-variants.json**: Contents of `.temp/react-to-figma-dom/components/{Name}/code-variants.json`
...
Also read the `slots` array from `code-variants.json`, filtering for entries where `independent: true`. These are NOT multiplied with the dependent axes — they get one story per non-default value, rendered at the default dependent-axis values.
...
**Dependent axes** — Generate one story per combination in the `combinations.dependent` array of `code-variants.json`. These are the full cross-product of dependent axes.

**Independent axes (Component Properties)** — Generate one story per non-default value, at the default dependent-axis values. These are listed in the `combinations.independent` array of `code-variants.json`.

Example: If `code-variants.json` lists:
...
// ... one export per variant from code-variants.json
```
