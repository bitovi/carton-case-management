...
BASELINE_FILE="${PIPELINE_DIR}/baseline-body.json"

SKIP_CAPTURE=false
VARIANTS_SOURCE="auto"
COMPONENT_LIST=()

for arg in "$@"; do
  if [[ "$arg" == "--skip-capture" ]]; then
    SKIP_CAPTURE=true
  elif [[ "$arg" == --variants-source=* ]]; then
    VARIANTS_SOURCE="${arg#--variants-source=}"
  elif [[ "$arg" == "--variants-source" ]]; then
    echo "ERROR: --variants-source requires a value: app|stories|auto"
    exit 1
  else
    COMPONENT_LIST+=("$arg")
  fi
...
if [[ "$VARIANTS_SOURCE" != "app" && "$VARIANTS_SOURCE" != "stories" && "$VARIANTS_SOURCE" != "auto" ]]; then
  echo "ERROR: Invalid --variants-source '$VARIANTS_SOURCE'. Use app, stories, or auto."
  exit 1
fi
...
if [[ "$SKIP_CAPTURE" == false && "$VARIANTS_SOURCE" != "app" ]]; then
  ...
  # Capture baseline fingerprints (once, before any component captures)
  if [[ ! -f "$BASELINE_FILE" ]]; then
    echo ""
    echo "📋 Capturing baseline body fingerprints..."
    mkdir -p "${COMPONENTS_DIR}/Baseline/variants"
    if node "$CAPTURE_SCRIPT" \
        --component Baseline \
        --storybook-url "$STORYBOOK_URL" \
        --output-dir "${COMPONENTS_DIR}/Baseline/variants" \
        --capture-baseline 2>&1; then
      # Move baseline file to pipeline root
      if [[ -f "${COMPONENTS_DIR}/Baseline/baseline-body.json" ]]; then
        mv "${COMPONENTS_DIR}/Baseline/baseline-body.json" "$BASELINE_FILE"
      fi
      echo "✓ Baseline fingerprints saved to ${BASELINE_FILE}"
    else
      echo "WARNING: Baseline capture failed — portal detection will use heuristic fallback"
    fi
  else
    echo "✓ Baseline fingerprints already exist at ${BASELINE_FILE}"
  fi
fi
...
if [[ ${#COMPONENT_LIST[@]} -gt 0 ]]; then
  ...
  COMPONENT_LIST=()
  if [[ "$VARIANTS_SOURCE" == "app" || "$VARIANTS_SOURCE" == "auto" ]]; then
    while IFS= read -r comp_name; do
      [[ -z "$comp_name" ]] && continue
      COMPONENT_LIST+=("$comp_name")
    done < <(
      find "${COMPONENTS_DIR}" -path "*/app-variants/capture-manifest.json" -type f 2>/dev/null \
        | awk -F'/' '{print $(NF-2)}' \
        | sort -u
    )
  fi

  if [[ "$VARIANTS_SOURCE" == "stories" || "$VARIANTS_SOURCE" == "auto" ]]; then
    while IFS= read -r comp_name; do
      [[ "$comp_name" == "Baseline" ]] && continue
      if [[ ! " ${COMPONENT_LIST[*]} " =~ " ${comp_name} " ]]; then
        COMPONENT_LIST+=("$comp_name")
      fi
    done < <(
...
process_component() {
  ...
  local STORY_VARIANTS_DIR="${COMP_DIR}/variants"
  local APP_VARIANTS_DIR="${COMP_DIR}/app-variants"
  local VARIANTS_DIR="${STORY_VARIANTS_DIR}"
  local STATUS_FILE="${COMP_DIR}/preprocess-status.json"

  if [[ "$VARIANTS_SOURCE" == "app" ]]; then
    VARIANTS_DIR="$APP_VARIANTS_DIR"
  elif [[ "$VARIANTS_SOURCE" == "auto" ]]; then
    if [[ -d "$APP_VARIANTS_DIR" ]] && { [[ -f "${APP_VARIANTS_DIR}/capture-manifest.json" ]] || [[ $(find "$APP_VARIANTS_DIR" -name "dom.json" 2>/dev/null | wc -l | tr -d ' ') -gt 0 ]]; }; then
      VARIANTS_DIR="$APP_VARIANTS_DIR"
    fi
  fi
  ...
  # ── Step 1: Capture DOM ────────────────────────────────────────────────
  if [[ "$VARIANTS_DIR" == "$APP_VARIANTS_DIR" ]]; then
    capture_status="from_app_variants"
    capture_captured=$(find "$VARIANTS_DIR" -name "dom.json" 2>/dev/null | wc -l | tr -d ' ')
    echo "  [capture] Using app-variants (${capture_captured} dom.json files)"
  elif [[ "$SKIP_CAPTURE" == true ]] && [[ -d "$VARIANTS_DIR" ]] && [[ -f "${VARIANTS_DIR}/capture-manifest.json" ]]; then
