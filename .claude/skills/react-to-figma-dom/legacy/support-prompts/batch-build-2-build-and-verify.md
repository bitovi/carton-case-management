# Batch Build and Verify

**Begin your response by outputting the heading above verbatim.**

Orchestrate building every preprocessed component in Figma and verifying the results. Spawns two subagents per component (build, then verify), tracks progress via a todo list, and aggregates results into a scoreboard.
...
Create a todo list with one item per component (e.g., `Build & Verify: Badge`, `Build & Verify: Button`, ...) plus a final `Run scoreboard` item. All items start as `not-started`.

Print the list of components you will process and their variant counts.
...
2. Spawn a **Build subagent** (see §Build Subagent below).
...
5. Print a one-line status: `{componentName}: {build_ok|build_failed} / {verify_verdict|skipped}`
...
```
Component           | Build    | Verify   | Avg Match%
--------------------|----------|----------|----------
Badge               | ok       | FAIL     | 61.2%
BaseEditable        | ok       | FAIL     | 71.3%
Button              | ok       | PASS     | 94.1%
...
```
...
```
Build component "{componentName}" in Figma.
...
These are ALL the variants to build — no filtering.

### 3. Execute each build-script.js — ONE AT A TIME, NO ANALYSIS

Process variants sequentially. For EACH variant:
...
### 4. Combine into component set (if >1 variant)
...
- componentName
- variantCount (number of variants successfully built)
