...
After the script completes, check the `capture-manifest.json`:

- If any captures have `status != "ok"`, diagnose why those variants failed (missing story, render timeout, etc.)
- Spot-check a few `dom.json` files to confirm they contain meaningful content (not just an empty root wrapper)
- Verify `.fiber-dom-map.json` files have component mappings (may be `"no-fiber"` for simple leaf components — that's expected)
