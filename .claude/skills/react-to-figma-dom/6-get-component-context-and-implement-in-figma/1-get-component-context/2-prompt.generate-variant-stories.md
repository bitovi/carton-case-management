...
### 7. Validate story is discoverable

After writing the story file, verify it appears in the Storybook index with the correct ID prefix:

```bash
sleep 3 && curl -s "http://localhost:6006/index.json" | python3 -c "
import json, sys
data = json.load(sys.stdin)
entries = data.get('entries', {})
prefix = 'figma-variants-{componentnamelowercase}--'
matches = [k for k in entries if k.startswith(prefix)]
if matches:
    print(f'✓ Found {len(matches)} stories with prefix {prefix}')
else:
    print(f'✗ ERROR: No stories found with prefix {prefix}')
    # Check for common wrong prefixes
    for k in entries:
        if '{componentnamelowercase}' in k and 'figma' in k:
            print(f'  Found under wrong ID: {k}')
    print('  → Fix the title field to: Figma Variants/{ComponentName}')
"
```

If the story is not found, fix the `title` field before proceeding.
