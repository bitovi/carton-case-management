...
- **Background**: `bg-*` class on the page root. Resolve through CSS variables if needed (see Step 1g of `.claude/skills/figma-from-code/7-build-component/SKILL.md` for the full chain).
...
If the page renders any Lucide icons directly (not via a child component), map each to its `builtComponents` entry (`Icon/{Name}`) and size from the className. See Step 1c of `.claude/skills/figma-from-code/7-build-component/SKILL.md` for the size mapping table.
...
See Step 1g of `.claude/skills/figma-from-code/7-build-component/SKILL.md` for the full decision tree: orchestrator-dispatched vs standalone vs auto/non-interactive, how to derive selectors, and when escalation to the user is required.
...
Use the same mapping table as `.claude/skills/figma-from-code/7-build-component/SKILL.md` Section 2d. Key entries for screen composition:
...
```json
{
  ...
  "trackingFile": {
    "written": true,
    "path": "packages/client/src/pages/CasesPage.figma-screen.json"
  }
}
```
