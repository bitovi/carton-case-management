...
try {
  ...
  const boundsA = getContentBounds(pngA);
  const boundsB = getContentBounds(pngB);
  ...
  const verdict       = matchPct >= MATCH_THRESHOLD ? 'match'
  ...
  const effectiveVerdict = borderVerdict === 'border_diff' && verdict === 'match'
    ? 'minor_diff' : verdict;

  const icon = effectiveVerdict === 'match' ? '✅' : effectiveVerdict === 'minor_diff' ? '⚠️' : '❌';
  console.log(`${icon} ${effectiveVerdict.toUpperCase()} — overall ${matchPct}% | border ${borderMatchPct}% (${borderDiff}/${borderTotal} border px differ)`);
