...
> **This step runs inside an Opus fix subagent with fresh context.** You have NO knowledge of how this component was built. Your diagnosis MUST start from `diff.png` and `comparison.json`, not from assumptions about what "should" be wrong. Read the source `.tsx` ONLY to resolve specific differences the comparison identified — never to form a prior about what the component "probably" looks like.
>
...
2. **Re-run the Step 4a gate** if the previous iteration's failure was `missing_instances`:
   ```bash
   node .claude/skills/figma-from-code-build-component/check-instances.js <componentName> <sourceDir>
   ```
