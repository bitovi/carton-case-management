...
> **Steps 4b and 4c run inside a Haiku compare subagent.** You receive structured inputs and produce a structured verdict. Run 4b (sizing check), then 4c (pixel diff). Return the verdict — do NOT attempt any fixes. If the verdict is not "match", the controller will dispatch a separate Opus fix subagent.
>
> **Step 4a runs inline in the controller** (before this subagent is dispatched) because it is a deterministic script gate that needs the node ID from Step 2.
...
```bash
node .claude/skills/figma-from-code-build-component/check-instances.js \
  <componentName> \
  <sourceDir>
```
...
```json
{
  ...
  "missingDetails": [
    { "figmaName": "EditableTitle", "sourceName": "EditableTitle", "origin": "@/components/inline-edit" }
  ],
